...
WXInfoEngine::InitChannelAndMode(void){
	/*channel_.push_back("“ª∫≈");
	channel_.push_back("∂˛∫≈");
	channel_.push_back("»˝∫≈");
	channel_.push_back("Àƒ∫≈");
	channel_.push_back("ŒÂ∫≈");
	channel_.push_back("¡˘∫≈");
	channel_.push_back("∆ﬂ∫≈");
	channel_.push_back("∞À∫≈");
	channel_.push_back("æ≈∫≈");
	channel_.push_back(" Æ∫≈");*/
...
WXInfoEngine::ProcessEventText(WXPacket& msg){
    ...
    msg.GetAttrib(EventKey,event_key);
	std::string content;
	if (str_event=="subscribe"){
        HttpShowWebFM(to_user,from_user);
	}else if(str_event=="CLICK"){
        if (event_key=="today_recommend") {
            HttpNewArticle(to_user,from_user);
        }else if(event_key=="migwebfm"){
            HttpShowWebFM(to_user,from_user);
        }else if(event_key=="channel_1"||event_key=="channel_2"||event_key=="channel_3"||
                 event_key=="channel_4"||event_key=="channel_5"||event_key=="channel_6"||
                 event_key=="channel_7"||event_key=="channel_8"||event_key=="channel_9"||
                 event_key=="channel_10"||event_key=="channel_11"||event_key=="channel_12"||
                 event_key=="channel_13"||event_key=="channel_14"||event_key=="channel_15"||
                 event_key=="channel_16"||event_key=="channel_17"||event_key=="channel_18"||
                 event_key=="channel_19"||event_key=="channel_20"){
            GetMenuChannelMusic(from_user,to_user,event_key);
        }else if(event_key=="recom_more"){
            HttpShowWebFM(to_user,from_user);
        }else if(event_key=="mm_r1"||event_key=="mm_r2"||event_key=="mm_r3"||
                 event_key=="mm_r4"||event_key=="mm_r5"||event_key=="mm_r6"||
                 event_key=="ms_r1"||event_key=="ms_r2"||event_key=="ms_r3"||
                 event_key=="ms_r4"||event_key=="ms_r5"||event_key=="ms_r6"){
            GetMoodSceneWordSongMenu(from_user,to_user,event_key);
        }else if(event_key=="last_recommend"){
            GetHistroyNews(from_user,to_user);
        }
    }
}
...
void
WXInfoEngine::HttpShowWebFM(std::string& to_user,std::string& from_user){
    std::string temp_msg;
    std::stringstream os;
    os<<"<xml><ToUserName><![CDATA["<<from_user.c_str()<<"]]></ToUserName>"
    <<"<FromUserName><![CDATA["<<to_user.c_str()<<"]]></FromUserName>"
...
WXInfoEngine::ProcessMsgText(WXPacket& msg){
	...
	}else if (content_s=="m"||content_s=="today_recommend"){
		HttpNewArticle(to_user,from_user);
		 ...
		 if (content_s=="lp1"){
             ...
             //url = "http://m1.file.xiami.com/17/2017/11020/136056_4394984_l.mp3";
	...
	}else if (content_s=="mm"||content_s=="ms"||content_s=="mn"){//–ƒ«È
		SetUserMode(from_user,to_user,content_s);
	}else{
		//ªÒ»°”√ªßµ±«∞◊¥Ã¨ –ƒ«È,mm ≥°æ∞ms ∆’Õ®mn
		std::string mode;
...
bool WXInfoEngine::SetUserMode(std::string& from_user,std::string to_user,
   ...
   content<<"«–ªªƒ£ Ω≥…π¶£¨ƒ˙“—æ≠«–ªªµΩ"<<it->second.c_str();
   size_t out_len;
   char* out;
   std::string str_out;
...
void WXInfoEngine::MigFMShow(std::string &msg,const int flag){
 	std::string title;
	if (flag){
		title = "Mig.FM 听对的音乐 场景推荐+心情推荐+频道推荐";
	}else{
		title = "Mig.FM 听对的音乐 场景推荐+心情推荐+频道推荐";
	}
 	std::string description = "最懂你的FM，听对的音乐 遇见对的人 定格属于你的音乐属性 发现的你音乐基因，点击阅读全文";
 	std::string pic = "http://42.121.14.108/wx/migfm.jpg";
 	std::string url = "121.199.32.88";
	HttpMigPicText(title,description,pic,url,msg,0);
}
...
void WXInfoEngine::GetWxToken(){
    Json::Reader reader;
	Json::Value  root;
    //获取凭证
     std::string token_url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=wxa784d1abb1d98bad&secret=1189440fb1aad9e740f599589f42cdec";
     
     wxinfo::HttpResponse http_token(token_url);
     http_token.Get();
     std::string token_content;
     http_token.GetContent(token_content);
     
     //解析token
     if (!token_content.empty()) {
         
         bool r = reader.parse(token_content.c_str(),root);
         if (!r){
             MIG_ERROR(USER_LEVEL,"parse json error[%s]",token_content.c_str());
             return ;
         }
         wx_token_ = root["access_token"].asString();
         expires_in_token_time_ = root["expires_in"].asInt();
         MIG_DEBUG(USER_LEVEL,"access_token[%s] expires_in[%d]",wx_token_.c_str(),expires_in_token_time_);
     }
}
    
    
void WXInfoEngine::CreateMenu(){
    /*std::string menu = "{\"button\":[{\"name\":\"pindao\",\"sub_button\":[{\"type\":\"click\",\"name\":\"hello word\",\"key\":\"V1001_HELLO_WORLD\"},{\"type\":\"click\",\"name\":\"zanyige\",\"key\":\"V1001_GOOD\"}]}]}";*/
    ...
    //音乐频道
    std::vector<base::ChannelInfo>  channel_mode;
    
    r = base_storage::MysqlSerial::GetRawChannelInfo(channel_mode);
    std::string channnel_name = "音乐频道";
    //char* utf_content = NULL;
    //size_t utf_content_size = 0;
    
    //base::BasicUtil::GB2312ToUTF8(content.c_str(),content.length(),&utf_content,&utf_content_size);
    
    os<<"{\"button\":[{\"name\":\""<<channnel_name.c_str()<<"\",\"sub_button\":[";
    
    if (r) {
        int flag = 0;
        for (int i = 0; i<5;i++) {
            os<<"{\"type\":\"click\",\"name\":\""<<channel_mode[i].channel_name().c_str()<<"\","
            <<"\"key\":\""<<"channel_"<<channel_mode[i].index().c_str()<<"\"}";
            if (i!=5-1) {
                os<<",";
            }
        }
    ...
    std::string recommend_name = "音乐推荐";
    os<<"{\"name\":\""<<recommend_name.c_str()<<"\",\"sub_button\":[";
    r = base_storage::MysqlSerial::GetDescriptionWord(word_list,1);
    if (r) {
        int flag = 0;
        while(word_list.size()>0&&recomm_index<2) {
            base::WordAttrInfo arrt_info = word_list.front();
            word_list.pop_front();
            recomm_index++;
            os<<"{\"type\":\"click\",\"name\":\""<<base64_decode(arrt_info.name()).c_str()
            <<"\","<<"\"key\":\""<<"mm_r"<<arrt_info.id().c_str()<<"\"}";
            os<<",";
        }
    }
    word_list.clear();
    
    r = base_storage::MysqlSerial::GetDescriptionWord(word_list,0);
    if (r) {
        recomm_index = 0;
        while(word_list.size()>0&&recomm_index<2) {
            base::WordAttrInfo arrt_info = word_list.front();
            ...
            recomm_index++;
            os<<"{\"type\":\"click\",\"name\":\""<<base64_decode(arrt_info.name()).c_str()
            <<"\","<<"\"key\":\""<<"ms_r"<<arrt_info.id().c_str()<<"\"}";
            os<<",";
        }
    }
    std::string more = "更多推荐";
    os<<"{\"type\":\"click\",\"name\":\""<<more.c_str()
    <<"\","<<"\"key\":\""<<"recom_more\"}";
    os<<"]},";
    
    
    os<<"{\"name\":\"音乐助手\",\"sub_button\":[";
    
    os<<"{\"type\":\"click\",\"name\":\"今日推荐文\",\"key\":\"today_recommend\"},"
      <<"{\"type\":\"click\",\"name\":\"历史推荐文\",\"key\":\"last_recommend\"},"
      <<"{\"type\":\"click\",\"name\":\"音乐推荐电台\",\"key\":\"migwebfm\"},"
      <<"{\"type\":\"click\",\"name\":\"使用说明\",\"key\":\"readme\"},"
      <<"{\"type\":\"click\",\"name\":\"关于我们\",\"key\":\"aboutus\"}";
    os<<"]}";
    
    os<<"]}";
    menu = os.str();
    
    MIG_DEBUG(USER_LEVEL,"%s",menu.c_str());
    std::string meun_url = "https://api.weixin.qq.com/cgi-bin/menu/create?access_token=";
    meun_url.append(wx_token_);
    
    MIG_DEBUG(USER_LEVEL,"meun_url[%s]",meun_url.c_str());
    wxinfo::HttpPost meun_post(meun_url);
    meun_post.Post(menu.c_str());
    std::string post_content;
    meun_post.GetContent(post_content);
    
    MIG_DEBUG(USER_LEVEL,"post_content[%s]",post_content.c_str());
}
    
void WXInfoEngine::GetMenuChannelMusic(std::string& from_user,
                                       std::string& to_user,
                                       const std::string& envent_key){
    std::string str_event_key;
    std::string durl;
    std::string hq_url;
	std::string title;
	std::string decs;
	...
	time_t current_time = time(NULL);
	base::ChannelInfo ci;
    str_event_key = envent_key;
    
    size_t nops = str_event_key.find("channel");
    if (nops==std::string::npos)
        return;
    int channel_num_len = str_event_key.length()-7-1;
    MIG_DEBUG(USER_LEVEL,"key_length[%d] nops[%d] channel_num_len[%d]",
              str_event_key.length(),nops,channel_num_len);
    
    std::string channel_num;
    channel_num = str_event_key.substr(7+1,channel_num_len);
    
    MIG_DEBUG(USER_LEVEL,"channel_num[%s]",channel_num.c_str());
    //key value: num|time|json
    MIG_DEBUG(USER_LEVEL,"key value: num|time|json");
    //key = "channel10000_";
  //  key.append("%s",channel_num.c_str());
    os<<"channel1000_"<<channel_num.c_str();
    key = os.str();
    
    MIG_DEBUG(USER_LEVEL,"================");
    
    MIG_DEBUG(USER_LEVEL,"key[%s]",key.c_str());
    
    r = GetMemMusicInfos(from_user,key,atol(channel_num.c_str()),
                         durl,hq_url,title,decs,json_time);
    ci = channel_mode_[atol(channel_num.c_str())-1];
    if ((!r)||(json_time<current_time)) {
        MIG_DEBUG(USER_LEVEL,"name[%s] index[%s] douban[%s]",ci.channel_name().c_str(),
                  ci.index().c_str(),ci.douban_index().c_str());
        r = HttpGetDoubanMusicInfo(content,atol(ci.douban_index().c_str()));
        if (r){
            time_t json_time = time(NULL)+(60*60*2);
            r = SetMemMusicInfos(key,1,content,json_time);
            r = ParseJson(0,content,from_user,durl,title,decs);
        }
        else{
            MIG_ERROR(USER_LEVEL,"GetDoubanMusic Error");
...
//key value: num|time|json
bool WXInfoEngine::GetMemMusicInfos(std::string& from_user,
    ...
    std::string content;
    base::ChannelInfo ci;
    r = base_storage::MemDicSerial::GetString(key.c_str(),
                                              key.length(),&value,
                                              &value_len);
    if (!r)
        return false;
    ...
    int num_pos = temp  - temp_value;
    std::string snum_pos;
    snum_pos.assign(temp_value,num_pos);
    num = atol(snum_pos.c_str());
    ...
    std::string current_time;
    current_time.assign(temp_value,time_pos);
    json_time = atol(current_time.c_str());
    ...
    content.assign(temp+1,value_len-8);
    MIG_DEBUG(USER_LEVEL,"content %s",content.c_str());
    r = ParseJson(num,content,from_user,durl,title,decs);
    ...
    if((num+1)<5)
        //r = SetMemMusicInfo(from_user,(num+1),atol(channel.c_str()),content,current);
        r = SetMemMusicInfos(key,num+1,content,json_time);
    else{
        ci = channel_mode_[current_channel-1];
        std::string s_content;
        r = HttpGetDoubanMusicInfo(s_content,atol(ci.douban_index().c_str()));
        if (r){
            time_t json_time = time(NULL)+(60*60*2);
            r = SetMemMusicInfos(key,1,s_content,json_time);
            r = ParseJson(0,s_content,from_user,durl,title,decs);
        }
        else{
            MIG_ERROR(USER_LEVEL,"GetDoubanMusic Error");
            return r;
...
bool WXInfoEngine::SetMemMusicInfos(const std::string& key,
                                    const int32 num, std::string& json,
                                    time_t json_time){
    std::stringstream os;
	bool r = false;
	os<<num<<"|"<<json_time<<"|"<<json;
	r = base_storage::MemDicSerial::SetString(key.c_str(),key.length(),
		                                      os.str().c_str(),os.str().length());
    return true;

}

bool WXInfoEngine::GetHistroyNews(std::string& from_user,std::string& to_user){
    std::list<base::NewsInfos> lists;
    bool r = base_storage::MysqlSerial::GetHistroyRecommend(0,lists);
    if (!r)
        return false;
    std::stringstream os;
	os<<"<xml><ToUserName><![CDATA["<<from_user.c_str()<<"]]></ToUserName>"
    ...
    <<"<CreateTime>"<<time(NULL)<<"</CreateTime>"
    <<"<MsgType><![CDATA[news]]></MsgType>"
    <<"<ArticleCount>"<<lists.size()<<"</ArticleCount><Articles>";
	//os<<"<item>";
    while (lists.size()>0) {
        std::string tempmsg;
        base::NewsInfos news;
        ...
        lists.pop_front();
        os<<"<item>";
        HttpMigPicText(base64_decode(news.title()),base64_decode(news.desc()),
                       base64_decode(news.pic()),base64_decode(news.url()),tempmsg,0);
        os<<tempmsg.c_str();
        os<<"</item>";
    }
...
bool WXInfoEngine::GetMoodSceneWordSongMenu(std::string& from_user,std::string& to_user,
                                                  std::string& content){
    std::string song_id;
    std::string music_info;
    base::MusicInfo smi;
    ...
    bool r = base_storage::RedisDicSerial::GetMusicMapRadom(content,song_id);
    if (!r)
        return false;
    r = base_storage::RedisDicSerial::GetMusicInfos(song_id,music_info);
    if (!r)
    ...
    r =  base_storage::MysqlSerial::GetMusicUrl(song_id,hq_content_url,content_url);
    if (!r)
        return false;
    ...
    os<<base64_decode(smi.artist().c_str())
      << " "<<base64_decode(smi.album_title()).c_str()
      <<" "<<smi.pub_time().c_str();
    desc = os.str();
    PackageMusicMsg(from_user,to_user,base64_decode(smi.title()).c_str(),desc,hq_content_url,content_url);
    return true;
}
