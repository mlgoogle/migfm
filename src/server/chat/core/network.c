...
int echo_forward(void *privates,int length,struct server *srv)
{
    ...
    while(1){
        ...
        if((length!=spconn->psc_rx_size)&&(spconn->psc_rx_state==STATE_NEW)){
           ...
           spconn->psc_rx_nob_left = spconn->psc_rx_nob_wanted - sizeof(int);
        ...
        spconn->psc_rx_nob_wanted = spconn->psc_rx_nob_left = 4;
        if(conn->offset+4>block->length){
            assert(conn->offset+4>block->length);
