...
static inline void echo_block_free(struct echo_block *block,
								   struct server* srv) {
...
static inline struct echo_conn *echo_conn_alloc(struct psock_conn *sc,
                                                struct server *srv) {
  struct echo_conn *ec;
...
static inline void echo_conn_free(struct echo_conn *conn,struct server *srv) {
  ...
  if (conn->block) {
    echo_block_decref(conn->block);
  }
  if (conn->packet) {
    echo_packet_free(conn->packet);
  }
  free(conn);
}
...
static void block_add_tail(struct echo_packet *packet, struct echo_block *block,
  ...
  if (packet->block_count == 0) {
    ...
    struct echo_block *first = packet->u.one_block.one_block;
...
int echo_post_recv(struct echo_conn *conn, int status,struct server* srv) {
  ...
  if (conn->block == NULL) {
    conn->block = echo_block_alloc(conn);
    if (conn->block == NULL) {
      return -ENOMEM;
    }
...
void output_packet(struct echo_packet *packet,struct server *srv) {
  //psock_conn_t *conn = packet->conn->private;
  int rc;
 /* while (1) {
  ...
  if (packet->block_count == 1) {
    ...
    echo_block_decref(packet->u.one_block.one_block);
  }
  else {
    int i;
    for (i = 0; i < packet->block_count; i++) {
      struct echo_block *block = 
        list_entry(packet->u.blocks.blocks.next, echo_block_t, list);
      list_del(&block->list);
      if (i == 0) {
...
int echo_forward(void *privates, int length,struct server* srv) {
  ...
  return echo_post_recv(conn, status);
}
...
struct psock_conn *create_sock_conn(struct server* srv) {
  ...
  conn = alloc_sock_conn();
  if (conn == NULL) {
    printf("alloc sock_conn failed\n");
    return NULL;
  }
...
again:  
  if (conn->psc_rx_nob_wanted != 0) {
    ...
    if (rc <= 0) {
      ...
      if (rc != -EAGAIN)
        sock_conn_decref(conn);

      return rc == 0?-ESHUTDOWN:rc;
    }
    assert(conn->psc_rx_nob_wanted == -1);

    rc = echo_forward(conn->psc_cookie, rc);
    if (rc == -EAGAIN) {
      goto again;
    }
  }
...
int sockbase_schedule(struct psock_sched *sched,struct server *srv) {
  ...
  while (1) {
    ...
    if (!list_empty(&sched->pss_rx_conns)) {
      ...
      rc = sockbase_process_receive(conn);
...
void server_read_buffer(int fd, short which, void *arg) {
  ...
  if (!conn->psc_rx_scheduled) { // not being progressed
  ...
  sockbase_schedule(conn->psc_scheduler);
...
struct sock_adapter *create_listen_socket(struct server* srv,const char* path){
  ...
  if (sa == NULL) {
    printf("alloc sock adapter failed\n");
    return NULL;
  }
  ...
  if (rc != 0) {
    printf("unable bind socket: %d\n", errno);
    goto rel_sock;
  }

  rc = listen(sock, 0);
  if (rc < 0) {
    printf("unable to listen to server socket: %d\n", errno);
    goto rel_sock;
  }
  ...
  if (rc != 0) {
    printf("fcntl set sock non-block failed: %d\n", errno);
    goto rel_sock;
  }

  sa->sock = sock;
  list_add_tail(&sa->list, sock_adapter_sock2hash(sock));

  return sa;
rel_sock:
  close(sock);
rel_sa:
  free_sock_adapter(sa);
  return NULL;
}
