...
// TODO: 设置请求类型
#define GET_TYPE		"weixin"
#define POST_TYPE		"weixin"
...
static void PostRequestMethod(std::string& content){
  ...
  post_content.append(dest_string);
  post_content.append("&type=" POST_TYPE "\n");

  //content.append("&type=" POST_TYPE "\n");
  MIG_INFO(USER_LEVEL,"%s",content.c_str());
  r = net::core_get(0,post_content.c_str(),post_content.length(),
	            respone,flag,code);
  if (!respone.empty()&&r)
	printf("Content-type: text/html\r\n"
		  "\r\n"
...
#if defined (__DEBUG_CORE__)
	std::string ipc_conn = "/var/www/tmp/debugcorefile";
#else
