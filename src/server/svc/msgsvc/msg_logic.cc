...
Msglogic::~Msglogic(){
	msgsvc_logic::DBComm::Dest();
	msgsvc_logic::DicComm::Dest();
...
bool Msglogic::Init(){
	...
	msgsvc_logic::DicComm::Init(config->redis_list_);
...
void Msglogic::GetPushMessageQueue(){
	...
	while(list.size()>0){
		...
		if (msg->MsgType()==0)
			...
			ResolveLeaveMessage(msg);

		list.pop_front();
	}
}

void Msglogic::ResolveLeaveMessage(base_queue::BlockMsg* block){
	//解析list 并存入
	//一个block 是一个用户行为
	std::string nickname;
	std::stringstream ss;
	...
	int64 uid = 0;
	int64 tid = 0;
	ResolveTemplate<msgsvc_logic::PushMessageInfo>(block,uid,tid,list);
	//写入数据库
	ss << nickname  << "给你留言啦";
...
void Msglogic::ResolveSayHello(base_queue::BlockMsg* block){
	...
	int64 tid = 0;
	ResolveTemplate<msgsvc_logic::PushMessageInfo>(block,uid,tid,list);
	msgsvc_logic::DBComm::RecordPushMessage(list);
	//写入数据库
	ss << nickname  << "给你打招呼";
	summary.assign(ss.str());
	PushMessage(uid,tid,summary);
...
void Msglogic::PushMessage(const int64 uid,const int64 tid,const std::string& message){
	//新增消息
	int64 new_msg = 1;
	msgsvc_logic::DicComm::GetNewCount(tid,new_msg);
