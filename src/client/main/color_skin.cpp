...
const TCHAR* const kAlphaColorSliderControlName = _T("alpha_controlor");
...
void ColorSkinWindow::Notify(TNotifyUI& msg)
{
	if (_tcsicmp(msg.sType, _T("click")) == 0)
	{
		...
		if (pTabControl != NULL)
		{
            ...
            {
                if (_tcsstr(msg.pSender->GetName(), _T("bkimg_")) != 0)
                {
                    CDuiString str = msg.pSender->GetName().Right(3);
                    DWORD dwImage = _atoi_l(str.GetData(), 0);

                    main_frame_->SetBkImge(dwImage);
                }
            }
		}
	}
	else if (_tcsicmp(msg.sType, _T("valuechanged")) == 0)
	{
		...
		if (pTabControl != NULL)
		{
			if (pTabControl->GetCurSel() == 0)
			{
                ...
                CSliderUI* AlphaColorSlider = static_cast<CSliderUI*>(m_PaintManager.FindControl(kAlphaColorSliderControlName));
                if (NULL != AlphaColorSlider)
                {
                    int nAlpha = 255 - AlphaColorSlider->GetValue()/2;
                    main_frame_->SetBkAlpha(nAlpha);
                }
			}
			else if (pTabControl->GetCurSel() == 1)
			{}
